...
Los scripts se protegen por su ruta completa, no bloqueando globalmente el intérprete. Guardian inspecciona la línea de comandos de `cmd.exe`, `py.exe`, `python.exe`, `pythonw.exe`, PowerShell o `pwsh` y solo intercepta el proceso cuando contiene el `.bat` o `.py` configurado, ya sea con su ruta completa, con un nombre relativo que se resuelve contra el directorio desde el que se lanzó (`python backup.py`, `.\backup.py`) o mediante una ruta corta 8.3 existente en un disco local, que Windows normaliza a su ruta larga antes de compararla (una ruta de red nunca se consulta y se compara tal como está). También reconoce una cadena de shell como `cmd /c "python backup.py"`, y divide la línea de comandos con el analizador propio de Windows para que las comillas escapadas no puedan ocultar el script. No detecta `python -m módulo` ni un script iniciado desde una cadena de shell que antes cambie de directorio; resolver cualquiera de ambos exigiría interpretar módulos de Python o el lenguaje de `cmd`/PowerShell y podría autorizar un archivo distinto. Tras validar la contraseña, `.bat` se relanza con `cmd.exe`; para `.py`, Guardian vuelve a iniciar el mismo intérprete en el directorio de trabajo original, incluidos entornos embebidos y virtuales, y conserva únicamente el script, los argumentos que lo siguen y las opciones inocuas del intérprete (`-u`, `-O`, `-X`, `-W`, selectores de `py` como `-3.11`). Descarta cualquier opción que ejecute otro código (`-c`, `-m`, `-i`) y las envolturas de shell, para que la aprobación de un script no autorice una carga distinta.
...
Para los archivos `.exe`, Guardian instala una puerta de ejecución por ruta completa mediante Image File Execution Options (IFEO). Windows inicia `ProtectedApp.Gate` en lugar de cargar la imagen protegida; Gate registra el intento en Guardian y termina sin ejecutar la aplicación. La misma puerta se instala sobre los lanzadores Python detectados en el perfil y sobre intérpretes embebidos encontrados cerca de cada `.py` protegido (por ejemplo `python_embeded`, `venv` o `.venv`). Gate comunica a Guardian cada inicio de `py.exe`/`python.exe`; Guardian bloquea únicamente el script protegido y vuelve a iniciar el resto con el mismo intérprete, argumentos y directorio. Ese reinicio lo hace Guardian en la sesión del usuario, por lo que el proceso no es hijo de quien lo invocó, parte de un entorno nuevo, no hereda la consola ni las redirecciones y no devuelve su código de salida. Si el intérprete no lo inició el usuario de la sesión interactiva (un servicio, otra cuenta o `runas`), Guardian no lo reinicia, para no ejecutarlo con otra identidad, y Gate muestra un aviso. Guardian aprende además cualquier intérprete Python nuevo que alcance la capa de respaldo, de modo que su siguiente ejecución pase por Gate. Después de validar la contraseña, Guardian mantiene abierta la familia del intérprete mientras existan procesos de esa ejecución, para que workers y subprocesos conserven su padre, consola y manejadores originales; al finalizar el último proceso restaura automáticamente las puertas. Durante ese intervalo, la suscripción inmediata a `Win32_ProcessStartTrace` continúa comprobando otros intentos protegidos. Si el servicio no responde, Gate falla de forma cerrada y el destino continúa sin iniciarse.
