param(
    [string]$Output = (Join-Path $PSScriptRoot 'ProtectedFolderUnlockedIcon.ico')
)
...
public static class ProtectedAppUnlockedFolderNativeIcons
{
    [DllImport("user32.dll", CharSet = CharSet.Unicode)]
    public static extern uint PrivateExtractIcons(string fileName, int iconIndex, int iconWidth,
        int iconHeight, IntPtr[] icons, uint[] iconIds, uint iconCount, uint flags);
...
function Add-UnlockedBadge([Drawing.Graphics]$Graphics, [int]$Size) {
    ...
    try {
        ...
        $Graphics.DrawString([string][char]0xE785, $font, [Drawing.Brushes]::White,
...
foreach ($size in $sizes) {
    ...
    if ([ProtectedAppUnlockedFolderNativeIcons]::PrivateExtractIcons($shellIconLibrary, 3, $size, $size, $handles, $iconIds, 1, 0) -ne 1 -or $handles[0] -eq [IntPtr]::Zero) {
        throw "No se pudo extraer el icono nativo de carpeta de Windows para $size px."
    }

    $folderIcon = $null; $folderBitmap = $null; $bitmap = $null; $graphics = $null
    try {
        ...
        Add-UnlockedBadge $graphics $size
        ...
        [void][ProtectedAppUnlockedFolderNativeIcons]::DestroyIcon($handles[0])
    }
}
