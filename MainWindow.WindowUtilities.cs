...
public sealed partial class MainWindow
{
    ...
    private SolidColorBrush ThemeBrush(Windows.UI.Color dark, Windows.UI.Color light)
        => new(Root.ActualTheme == ElementTheme.Light ? light : dark);
    ...
    private IntPtr EnforceMinimumWindowSize(IntPtr hwnd, uint message, IntPtr wParam, IntPtr lParam, UIntPtr id, UIntPtr data)
    {
        ...
        if (message == WmGetMinMaxInfo && lParam != IntPtr.Zero)
        {
            ...
            sizing.MinimumTrackSize.X = Math.Max(sizing.MinimumTrackSize.X, (int)Math.Ceiling(MinimumWindowWidth * scale));
            sizing.MinimumTrackSize.Y = Math.Max(sizing.MinimumTrackSize.Y, (int)Math.Ceiling(MinimumWindowHeight * scale));
